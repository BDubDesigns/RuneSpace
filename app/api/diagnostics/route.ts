...
function underLimit(request: Request) {
  ...
  if (!visitors.has(key) && visitors.size >= MAX_CLIENTS) visitors.delete(visitors.keys().next().value!);
  const value = visitors.get(key);
  if (!value || value.resetAt <= now) { visitors.set(key, { count: 1, resetAt: now + WINDOW_MS }); return true; }
  value.count += 1;
...
export async function POST(request: Request) {
  ...
  if (!underLimit(request)) return new NextResponse(null, { status: 429, headers: { ...noStore, "retry-after": "60" } });
  const contentLength = Number(request.headers.get("content-length") ?? 0);
  if (contentLength > DIAGNOSTIC_MAX_BYTES) return new NextResponse(null, { status: 413, headers: noStore });
  try {
    const body = await request.text();
    if (new TextEncoder().encode(body).byteLength > DIAGNOSTIC_MAX_BYTES) return new NextResponse(null, { status: 413, headers: noStore });
  ...
  } catch { return new NextResponse(null, { status: 400, headers: noStore }); }
}
