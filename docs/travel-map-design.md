...
- **Layer 2 — decorative identifier:** single-file helper resolves `mapIconKey → /map-icons/…` asset.
  ...
  `x/y/width≈62% hexW / height≈46% hexH`, `preserveAspectRatio="xMidYMid meet"` and `opacity="0.32"`.
  Whole group is `aria-hidden`. Size 55–65% hex width, centered slightly above center so the lower nameplate
  keeps contrast. Does not change bounds, does not overlap routes, does not hide `data-route-progress`.
...
- New locations **must** provide a compact identifier through the same `mapIconKey → helper` boundary and a local
  `public/map-icons/<slug>.png` (or `.webp`) asset; no baked text/status in art; opaque background required to be
  removed / made subordinate; respect hex zones and 55–65% / ≤ `0.38` opacity rules.

## Assets (approved, local-only)
- `public/map-icons/crash-site.png` — Crash Site (wrecked hull, from `img_638277192bac.png`), 1536×1536, ~1.41 MB
- `public/map-icons/processing-yard.png` — Processing Yard (hopper/conveyor/gantry/crusher, from `img_57cb431b0abe.png`), 1536×1536, ~1.25 MB
- `public/map-icons/power-annex.png` — Power Annex (battery rack/power core, from `img_82b4c98005fa.png`), 1536×1536, ~1.43 MB
- PNG kept because transparent WebP recompression on this box would risk tonal fidelity without a verified pipeline;
  defer WebP only when fidelity-safe and sizes remain reviewable. No remote URLs, no placeholders, no substitutions.
