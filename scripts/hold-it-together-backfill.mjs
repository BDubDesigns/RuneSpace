...
// One-time Issue #148 maintenance operation.
//
// The default command is a read-only dry run. Execution requires both a saved
// dry-run report and an exact confirmation token. This repair adds only the
// accepted Hold It Together mission row; it never touches Cargo, Welding, XP,
// inventory, or mission-progress rows.
...
export const REPORT_KIND = "runespace.issue-148.hold-it-together-backfill";
export const REPORT_SCHEMA_VERSION = 1;
export const EXECUTION_CONFIRMATION = "ISSUE-148-HOLD-IT-TOGETHER-BACKFILL";

const AUTHORITY = Object.freeze({
  missionIds: Object.freeze({
    wasteNot: MISSION_IDS.wasteNot,
    holdItTogether: MISSION_IDS.holdItTogether,
  }),
});

const USAGE = `Usage:
  pnpm --silent run maintenance:issue-148
  node --experimental-strip-types scripts/hold-it-together-backfill.mjs --verify --expected-report <dry-run.json>
  node --experimental-strip-types scripts/hold-it-together-backfill.mjs --execute --confirm ${EXECUTION_CONFIRMATION} --expected-report <dry-run.json>
...
export class HoldItTogetherBackfillError extends Error {
  constructor(message) {
    super(message);
    this.name = "HoldItTogetherBackfillError";
  }
}

function fail(message) {
  throw new HoldItTogetherBackfillError(message);
}
...
function assertReportShape(report) {
  ...
  if (stableJson(report.authority) !== stableJson(AUTHORITY))
    fail("expected report uses different canonical mission IDs");
...
export async function queryScan(client) {
  const [wouldAccept, alreadyAccepted] = await Promise.all([
    client.query(
      `
        SELECT c.id
        ...
        INNER JOIN character_missions AS waste
         ...
         AND waste.mission_id = $1
         AND waste.completed_at IS NOT NULL
        LEFT JOIN character_missions AS hold
          ON hold.character_id = c.id
         AND hold.mission_id = $2
        WHERE hold.character_id IS NULL
        ORDER BY c.id
      `,
      [AUTHORITY.missionIds.wasteNot, AUTHORITY.missionIds.holdItTogether],
    ),
    client.query(
      `
        SELECT c.id
        ...
        INNER JOIN character_missions AS waste
         ...
         AND waste.mission_id = $1
         AND waste.completed_at IS NOT NULL
        INNER JOIN character_missions AS hold
          ON hold.character_id = c.id
         AND hold.mission_id = $2
        ORDER BY c.id
      `,
...
function assertExpectedReportMatches(scan, expectedReport) {
  ...
  const actualCohort = [
    ...scan.wouldAcceptCharacterIds,
    ...scan.alreadyAcceptedCharacterIds,
  ].sort();
  ...
  if (
    stableJson(actualCohort) !== stableJson(expectedCohort) ||
    stableJson([...scan.wouldAcceptCharacterIds].sort()) !== stableJson(expectedPending)
  ) {
...
export async function applyBackfill(client, characterIds, now) {
  ...
  await client.query(
    ...
    `,
    [characterIds, AUTHORITY.missionIds.holdItTogether, now],
  );
}
...
export async function verifyApplied(client, expectedReport) {
  ...
  const rows = await client.query(
    `
      SELECT cm.character_id, cm.accepted_at, cm.completed_at, p.progress_key
      ...
      WHERE cm.mission_id = $1
        AND cm.character_id = ANY($2::text[])
      ORDER BY cm.character_id
    `,
    [AUTHORITY.missionIds.holdItTogether, expectedReport.wouldAcceptCharacterIds],
  );
  ...
  const passed = expectedReport.wouldAcceptCharacterIds.every((characterId) => {
    ...
    return row && row.accepted_at && !row.completed_at && row.progress_key === null;
  });
  return {
    expected: expectedReport.wouldAcceptCharacterIds.length,
    accepted: new Set(rows.rows.map((row) => row.character_id)).size,
    completed: rows.rows.filter((row) => row.completed_at).length,
    progressRows: rows.rows.filter((row) => row.progress_key !== null).length,
    passed,
  };
}
