...
- This directory is excluded from the Docker build context via the
  `.dockerignore` rule `assets` (which covers every art-master category);
  the Dockerfile asserts the masters are absent after `COPY . .`.
